...
//加载文件上传插件导入功能实现不上传服务器
function initImportFileUpload(){
	$("#importFileUploader").uploadFile({
		url:"test/import",
		maxFileCount: 5,                //上传文件个数（多个时修改此处
	    returnType: 'json',              //服务返回数据
	    allowedTypes: 'excel,xlsx,xls',  //允许上传的文件式
	        ...
	        $.ajax({
	            ...
	            success: function(data) 
	            {
	                if(data.data=="success"){
	                    ...
	                    $('#image').val('');
	                    var urls = $('#importAddForm [name=file]').val().split(",");  //将删除的文件url从urls中移除
	                    var deletedUrls = [];
	                	for(var i in urls){
	                		if(urls[i] != fileUrl){
	                	...
	                	$('#importAddForm [name=file]').val(deletedUrls);
	                 }else{
	    ...
	    onSuccess: function(files,data,xhr,pd)
	    {
	        ...
	        if(data&&data.error==0){
	        	...
	        	if( $('#importAddForm [name=file]').val() != null && $('#importAddForm [name=file]').val() != ''){
	        		/* alert($('#orderAddForm [name=file]').val()); */
	        		$('#importAddForm [name=file]').val($('#importAddForm [name=file]').val()+","+data.url);
	        	}else{
	            	$('#importAddForm [name=file]').val(data.url);
	        	}
	        }
	    }
	});
